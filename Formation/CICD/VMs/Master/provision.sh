...
# Dirty way to create keys and deploy them on the slaves to allow ssh connection when playing ansible playbook
yes y | ssh-keygen -q -t rsa -n '' -f ~/.ssh/id_rsa >/dev/null
sudo apt install -y sshpass
sshpass -p "vagrant" ssh-copy-id -o StrictHostKeyChecking=no vagrant@192.168.33.101
sshpass -p "vagrant" ssh-copy-id -o StrictHostKeyChecking=no vagrant@192.168.33.102
